...
const filePath = path.join(__dirname, 'frontend/src/pages/AdTracker.tsx');
...
const stateRegex = /const \[form, setForm\] = useState<Omit<Ad,'id'>>\(\{[\s\S]*?month:'', campaign:'', spend:0, reach:0, clicks:0, platform:'Facebook', status:'active', notes:''[\s\S]*?\}\);/;
const stateInjection = `const [form, setForm] = useState<Omit<Ad,'id'>>({
...
const addCampaignBtnRegex = /<button onClick=\{\(\)=>setShowForm\(true\)\}[\s\S]*?<Plus size=\{16\}\/> \{t\('adTracker\.addCampaign', 'Add Campaign'\)\}[\s\S]*?<\/button>/;
...
content = content.replace(addCampaignBtnRegex, newButtons);

fs.writeFileSync(filePath, content, 'utf8');
console.log('Successfully patched AdTracker.tsx with PDF generation');
