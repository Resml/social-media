...
const filePath = path.join(__dirname, 'frontend/src/pages/TeamTasks.tsx');
...
const stateRegex = /const \[form, setForm\] = useState<Omit<Task,'id'>>\(\{ title:'', category:CATEGORIES\[0\], assignee:TEAM\[0\], priority:'medium', status:'todo', dueDate:'' \}\);/;
const stateInjection = `const [form, setForm] = useState<Omit<Task,'id'>>({ title:'', category:CATEGORIES[0], assignee:TEAM[0], priority:'medium', status:'todo', dueDate:'' });
...
const addTaskBtnRegex = /<button onClick=\{\(\)=>setShowForm\(true\)\}[\s\S]*?<Plus size=\{16\}\/> \{t\('teamTasks\.addTask', 'Add Task'\)\}[\s\S]*?<\/button>/;
...
content = content.replace(addTaskBtnRegex, newButtons);

fs.writeFileSync(filePath, content, 'utf8');
console.log('Successfully patched TeamTasks.tsx with PDF generation');
