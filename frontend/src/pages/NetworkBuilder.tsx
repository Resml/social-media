...
const INITIAL: Contact[] = [
  { id:'1', name:'Ramesh Patil',    location:'Ward 12', interest:'Politics',          platform:'Facebook',  phone:'9876543210', tag:'Supporter',  note:'Active volunteer' },
  { id:'2', name:'Sunita Deshmukh', location:'Ward 14', interest:'Education',         platform:'Facebook',  phone:'9765432109', tag:'Influencer', note:'School principal, 5k followers' },
  { id:'3', name:'Anil Kadam',      location:'Ward 12', interest:'Youth',             platform:'Instagram', phone:'9654321098', tag:'Activist',   note:'Youth group leader' },
  { id:'4', name:'Priya Shinde',    location:'Ward 15', interest:'Women Empowerment', platform:'Facebook',  phone:'9543210987', tag:'Voter',      note:'' },
  { id:'5', name:'Vijay Mane',      location:'Ward 12', interest:'Sports',            platform:'Facebook',  phone:'9432109876', tag:'Supporter',  note:'Cricket club organizer' },
];
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-black" style={{fontFamily:'Outfit,sans-serif',color:'var(--slate-900)'}}>Network Builder</h1>
              <p className="text-sm" style={{color:'var(--slate-500)'}}>Build and manage supporters, influencers and voter network</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 mb-5">
          {TAGS.map(t=>{
            const st=TAG_STYLE[t]; const cnt=contacts.filter(c=>c.tag===t).length;
            return (
              <button key={t} onClick={()=>setFilterTag(filterTag===t?'All':t)}
                ...
                style={{background:filterTag===t?st.bg:'#fff', borderColor:filterTag===t?st.color:'var(--slate-200)', borderWidth:filterTag===t?2:1}}>
                <div className="font-black text-lg leading-none mb-0.5" style={{color:filterTag===t?st.color:'var(--slate-600)'}}>{cnt}</div>
                <div className="text-[10px] font-bold leading-tight" style={{color:'var(--slate-500)'}}>{t}</div>
              </button>
            );
          })}
        </div>
        ...
        <div className="flex gap-3 mb-5">
          <div className="flex-1 relative">
            ...
            <input style={{...inputStyle,paddingLeft:'2.5rem'}} placeholder="Search by name, location or interest…" value={search} onChange={e=>setSearch(e.target.value)}/>
          </div>
          {filterTag!=='All'&&(
            ...
            return (
              <div key={c.id} className="bg-white rounded-2xl border p-4 group transition-all hover:shadow-md hover:-translate-y-0.5" style={{borderColor:'var(--slate-200)'}}>
                ...
                <div className="flex flex-wrap gap-1.5 mb-3">
                  <span className="text-[11px] font-bold px-2 py-0.5 rounded-full border" style={{background:st.bg,color:st.color,borderColor:st.border}}>{c.tag}</span>
                  <span className="text-[11px] font-bold px-2 py-0.5 rounded-full" style={{background:'var(--slate-100)',color:'var(--slate-600)'}}>{c.interest}</span>
                  <span className="text-[11px] font-bold px-2 py-0.5 rounded-full" style={{background:'var(--brand-50)',color:'var(--brand-700)'}}>{c.platform}</span>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{background:'rgba(15,23,42,0.5)', backdropFilter:'blur(4px)'}}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-bold text-lg" style={{fontFamily:'Outfit,sans-serif',color:'var(--slate-900)'}}>Add Contact</h3>
              <button onClick={()=>setShowForm(false)} className="p-2 rounded-xl hover:bg-slate-100"><X size={18}/></button>
            </div>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-xs font-bold uppercase tracking-widest mb-1.5" style={{color:'var(--slate-400)'}}>Full Name</label>
                  <input style={inputStyle} placeholder="Name…" value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))}/></div>
                <div><label className="block text-xs font-bold uppercase tracking-widest mb-1.5" style={{color:'var(--slate-400)'}}>Location / Ward</label>
                  <input style={inputStyle} placeholder="Ward / Area…" value={form.location} onChange={e=>setForm(f=>({...f,location:e.target.value}))}/></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-xs font-bold uppercase tracking-widest mb-1.5" style={{color:'var(--slate-400)'}}>Interest</label>
                  <select style={{...inputStyle,cursor:'pointer'}} value={form.interest} onChange={e=>setForm(f=>({...f,interest:e.target.value}))}>
                    {INTERESTS.map(i=><option key={i}>{i}</option>)}</select></div>
                <div><label className="block text-xs font-bold uppercase tracking-widest mb-1.5" style={{color:'var(--slate-400)'}}>Platform</label>
                  <select style={{...inputStyle,cursor:'pointer'}} value={form.platform} onChange={e=>setForm(f=>({...f,platform:e.target.value}))}>
                    <option>Facebook</option><option>Instagram</option><option>WhatsApp</option><option>None</option></select></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-xs font-bold uppercase tracking-widest mb-1.5" style={{color:'var(--slate-400)'}}>Phone</label>
                  <input style={inputStyle} placeholder="Phone number…" value={form.phone} onChange={e=>setForm(f=>({...f,phone:e.target.value}))}/></div>
                <div><label className="block text-xs font-bold uppercase tracking-widest mb-1.5" style={{color:'var(--slate-400)'}}>Tag</label>
                  <select style={{...inputStyle,cursor:'pointer'}} value={form.tag} onChange={e=>setForm(f=>({...f,tag:e.target.value as Tag}))}>
                    {TAGS.map(t=><option key={t}>{t}</option>)}</select></div>
              </div>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={addContact} className="flex-1 py-3 rounded-xl font-bold text-sm text-white hover:opacity-90 active:scale-95" style={{background:'var(--brand-600)'}}>Add Contact</button>
            </div>
          </div>
        </div>
      )}
