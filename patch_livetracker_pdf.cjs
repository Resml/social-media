...
const stateRegex = /const \[form, setForm\] = useState<Omit<LiveSession,'id'>>\(\{ topic:'', platform:'Facebook', via:'Direct', scheduledAt:'', duration:30, status:'scheduled', notes:'' \}\);/;
const stateInjection = `const [form, setForm] = useState<Omit<LiveSession,'id'>>({ topic:'', platform:'Facebook', via:'Direct', scheduledAt:'', duration:30, status:'scheduled', notes:'' });
...
const createLiveBtnRegex = /<button onClick=\{\(\)=>setShowForm\(true\)\}[\s\S]*?<Plus size=\{16\}\/> \{t\('liveTracker\.scheduleLive', 'Schedule Live'\)\}[\s\S]*?<\/button>/;
...
content = content.replace(createLiveBtnRegex, newButtons);

fs.writeFileSync(filePath, content, 'utf8');
console.log('Successfully patched LiveTracker.tsx with PDF generation');
