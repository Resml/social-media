...
function processFile(filePath, stateSearch, stateReplace, returnSearch, returnReplace, btnSearch, btnReplace) {
  let content = fs.readFileSync(filePath, 'utf8').replace(/\r\n/g, '\n');

  if (!content.includes('import * as htmlToImage')) {
    content = content.replace(
      "import { useTranslation } from 'react-i18next';",
      "import { useTranslation } from 'react-i18next';\nimport * as htmlToImage from 'html-to-image';\nimport { jsPDF } from 'jspdf';\nimport { toast } from 'sonner';\nimport { " + path.basename(filePath, '.tsx') + "Report } from '../components/" + path.basename(filePath, '.tsx') + "Report';\nimport { useRef } from 'react';\nimport { Download } from 'lucide-react';"
    );
  }
